...
class ProcessCommand: Command {
  ...
  func execute() throws {
    ...
    if fileManager.fileExists(atPath: supportedFilePath), !overwrite.value {
      🖨️("\(supportedFilePath) already exists. Do you want to overwrite? Y/N")

      if let userInput = readLine(), userInput.lowercased() != "y" {
        🖨️("File \(supportedFilePath) was not overwritten.")
        return
      }
    }

    if fileManager.fileExists(atPath: unsupportedFilePath), !overwrite.value {
      🖨️("\(unsupportedFilePath) already exists. Do you want to overwrite? Y/N")
      if let userInput = readLine(), userInput.lowercased() != "y" {
        🖨️("File \(unsupportedFilePath) was not overwritten.")
        return
      }
    }
...
func testSwiftCode(swiftCode: String) throws -> Bool {
  ...
  do {
    ...
    🖨️("An error occurred: \(error.localizedDescription)")
    throw error
  }
}
...
func main(jsonDataPath: String, supportedFilePath: String, unsupportedFilePath: String, resultsFilePath: String, resultsUnsupportedFilePath: String) throws {
  ...
  do {
    ...
    for item in swiftCodes {
      ...
      🖨️("\r\u{001B}[1;33mTesting: \u{001B}[0;0m \(count) of \(len)", terminator: "\r")
...
if let swiftVersion = getSwiftVersion() {
    ...
    🖨️("Unable to retrieve Swift version.")
}
    ...
    🖨️("Done! Check supported.swift and unsupported.swift for the results.")
  } catch {
    🖨️("An error occurred: \(error.localizedDescription)")
  }
}
